...
async function testDockerPullWithIntegration() {
  console.log("Testing Docker pull with AWS ECR integration...\n");
  ...
  const registryUrl = process.env.ECR_REPOSITORY_URI.split('/')[0];
  const specVersion = 8;
  const appName = 'ecr-test-app';

  try {
    // Step 1: Get credentials from integration
    ...
    if (pullOutput.includes("Pull complete") || pullOutput.includes("Image is up to date")) {
      ...
      const images = execSync(
        `docker images --format "{{.Repository}}:{{.Tag}}" | grep "${process.env.ECR_REPOSITORY_URI}"`,
        { encoding: "utf8" }
      );
