...
async function testDockerPullWithIntegration() {
  console.log("Testing Docker pull with Google Artifact Registry integration...\n");
  ...
  try {
    ...
    console.log("1. Getting Google GAR credentials from integration layer...");
    ...
    if (pullOutput.includes("Pull complete") || pullOutput.includes("Image is up to date")) {
      ...
      const images = execSync(
        `docker images --format "{{.Repository}}:{{.Tag}}" | grep "${process.env.GAR_REPOSITORY_URI}"`,
        { encoding: "utf8" }
      );
      ...
      const containerName = "flux-gar-test-container";
