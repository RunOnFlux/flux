...
function Config() {
  ...
  if (platform === 'linux') {
    ...
    let pathMac =
        nodePath.join(homedir, 'Library', 'Application Support', coinpathmac);
    const pathMacNew = nodePath.join(homedir, 'Library', 'Application Support',
                                     newcoinpathmac);
  ...
  this.filteredlines =
      fs.readFileSync(confFilePath, 'utf-8')
          .split('\n')
  ...
  this.keysvalues = this.filteredlines.reduce((map, line) => {
    ...
    if (sp.length === 2) {
      ...
      map[sp[0].trim()] =
          `${sp[1].trim()}=${sp[2].trim()}=${sp[3].trim()}=${sp[4].trim()}`;
    } else if (sp.length === 6) {
      map[sp[0].trim()] = `${sp[1].trim()}=${sp[2].trim()}=${sp[3].trim()}=${
          sp[4].trim()}=${sp[5].trim()}`;
    }
...
Config.prototype.defaultFolder = function() { return this.defaultFolderPath; };
Config.prototype.defaultConfig = function() { return this.defaultConfigPath; };
Config.prototype.get = function(name) { return this.keysvalues[name]; };
Config.prototype.rpcuser = function() { return this.keysvalues.rpcuser; };
Config.prototype.rpcpassword =
    function() { return this.keysvalues.rpcpassword; };
Config.prototype.rpcport = function() { return this.keysvalues.rpcport; };
module.exports = {
  Config,
};
