...
function Config() {
  ...
  if (platform === 'linux') {
    ...
    let pathMac = nodePath.join(
      homedir,
      'Library',
      'Application Support',
      coinpathmac
    );
    const pathMacNew = nodePath.join(
      homedir,
      'Library',
      'Application Support',
      newcoinpathmac
  ...
  this.filteredlines = fs
    .readFileSync(confFilePath, 'utf-8')
    .split('\n')
  ...
  this.keysvalues = this.filteredlines.reduce((map, line) => {
    ...
    if (sp.length === 2) {
      ...
      map[
        sp[0].trim()
      ] = `${sp[1].trim()}=${sp[2].trim()}=${sp[3].trim()}=${sp[4].trim()}=${sp[5].trim()}`;
    }
    return map;
  }, {});
}
