...
function resolveConfigPath() {
  ...
  const fluxdEnvPath = process.env.FLUXD_CONFIG_PATH;

  if (fluxdEnvPath) {
    const stats = fs.statSync(fluxdEnvPath, { throwIfNoEntry: false });
    if (stats && stats.isFile()) {
      return [nodePath.dirname(fluxdEnvPath), fluxdEnvPath];
    }
  }
  ...
  return [folderPath, confFilePath];
}

function Config() {
  const [defaultFolderPath, defaultConfigPath] = resolveConfigPath();
  this.defaultFolderPath = defaultFolderPath;
  this.defaultConfigPath = defaultConfigPath;

  // Read all lines from config file, remove commented lines and empty lines.
  this.filteredlines = fs.readFileSync(defaultConfigPath, 'utf-8').split('\n').filter((line) => line.trim() && !line.trim().startsWith('#'));
