...
async function connectMongoDb(url) {
  ...
  const db = await MongoClient.connect(connectUrl, mongoSettings).catch(
    (error) => {
      throw error;
    }
  );
...
async function verifyPrivilege(privilege, req, appName) {
  ...
  switch (privilege) {
    case 'admin':
      authorized = await verifyAdminSession(req.headers).catch((error) => {
        throw error;
      });
      break;
    case 'zelteam':
      authorized = await verifyZelTeamSession(req.headers).catch((error) => {
        throw error;
      });
      break;
    case 'adminandzelteam':
      authorized = await verifyAdminAndZelTeamSession(req.headers).catch(
        (error) => {
          throw error;
        }
      );
      break;
    case 'appownerabove':
      authorized = await verifyAppOwnerOrHigherSession(
        req.headers,
        appName
      ).catch((error) => {
        throw error;
      ...
      break;
    case 'appowner':
      authorized = await verifyAppOwnerSession(req.headers, appName).catch(
        (error) => {
          throw error;
        }
      );
      break;
    case 'user':
      authorized = await verifyUserSession(req.headers).catch((error) => {
        throw error;
      });
      break;
    default:
