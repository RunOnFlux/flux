...
async function storeZelAppRunningMessage(message) {
  ...
  const queryFind = {
    ...
    broadcastedAt : newZelAppRunningMessage.broadcastedAt
  };
  ...
  req.on('end', async () => {
    try {
      ...
      if (!zelAppSpecification || !timestamp || !signature || !messageType ||
          !typeVersion) {
        throw new Error(
            'Inclomplete message received. Check if specifications, timestamp and siganture is provided.');
      }
      ...
      if (portResult) {
        throw new Error(`ZelApp ${
            zelAppSpecFormatted
                .name} port already registered. ZelApp has to be registered with different port.`);
      }
...
async function availableZelApps(req, res) {
  ...
  const zelapps = [
    ...
    {
      name : 'SuperMario', // corresponds to docker name and this name is stored
                           // in zelapps mongo database
      description : 'LoL SuperMario',
      repotag : 'pengbai/docker-supermario:latest',
      owner : '1CbErtneaX2QVyUfwU7JGB7VzvPgrgc3uC',
      port : 30001,
      tiered : false,
      cpu : 0.2, // true resource registered for app. If not tiered only this is
                 // available
      ram : 200, // true resource registered for app
      hdd : 1,   // true resource registered for app
      enviromentParameters : [],
    ...
    {
      name : 'PacMan', // corresponds to docker name and this name is stored in
                       // zelapps mongo database
      description : 'LoL PacMan',
      repotag : 'uzyexe/pacman:latest',
      owner : '1CbErtneaX2QVyUfwU7JGB7VzvPgrgc3uC',
      port : 30002,
...
async function trySpawningGlobalApplication() {
  try {
    ...
    if (runningAppList.length >= 5) {
      throw new Error(`Application ${randomApp} is already spawned on ${
    ...
    if (runningApps.data.find(
      ...
      throw new Error(
          `${randomApp} application is already running on this Flux`);
    }
    ...
    for (const stoppedApp of stoppedApps) {
      try {
        ...
        if (appDetails) {
          ...
          // it is a stopped global zelapp. Try to run it. TODO If Fail remove
          // but in better way
