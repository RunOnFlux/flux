  ...
  try {
    ...
    for (const installedApp of installedApps) {
      ...
      if (result && result.hash !== installedApp.hash) {
        // eslint-disable-next-line no-await-in-loop
        log.warn(
          `Application ${installedApp.name} version is obsolete. Reinstalling.`
        );
        ...
        // eslint-disable-next-line no-await-in-loop
        await serviceHelper.delay(config.zelapps.removal.delay * 1000); // wait for delay mins so we dont have
        // more removals at the same time
        // check if node is capable to run it according to specifications
        // get app specifications
        // eslint-disable-next-line no-await-in-loop
        const appSpecifications = await getApplicationSpecifications(
          result.name
        );
        if (!appSpecifications) {
          throw new Error(
...
async function getAppPrice(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      const respondPrice = serviceHelper.createDataMessage(
        Math.ceil(actualPriceToPay)
      );
