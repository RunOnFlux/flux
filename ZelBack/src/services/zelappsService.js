...
async function getGlobalZelAppsSpecifications(req, res) {
  ...
  const results =
      await serviceHelper
          .findInDatabase(database, globalZelAppsInformation, query, projection)
          .catch((error) => {
            const errMessage = serviceHelper.createErrorMessage(
                error.message, error.name, error.code);
            res.json(errMessage);
...
async function storeZelAppTemporaryMessage(message,
  ...
  const result = await serviceHelper
                     .findOneInDatabase(database, globalZelAppsTempMessages,
                                        query, projection)
                     .catch((error) => {
                       log.error(error);
                       throw error;
  ...
  await serviceHelper
      .insertOneToDatabase(database, globalZelAppsTempMessages, value)
      .catch((error) => {
        log.error(error);
        throw error;
      });
