...
async function createZelAppVolume(zelAppSpecifications, res) {
  ...
  let execDD = `sudo dd if=/dev/zero of=${useThisVolume.mount}/${
      zelAppSpecifications.name}TEMP bs=107374182 count=${
  ...
  let execFS = `sudo mke2fs -t ext4 ${useThisVolume.mount}/${
      zelAppSpecifications.name}TEMP`;
  if (useThisVolume.mount === '/') {
    execFS = `sudo mke2fs -t ext4 ${useThisVolume.mount}tmp/${
        zelAppSpecifications.name}TEMP`;
  }
  ...
  let execRemoveAlloc =
      `sudo rm -rf ${useThisVolume.mount}/${zelAppSpecifications.name}TEMP`;
  if (useThisVolume.mount === '/') {
    execRemoveAlloc = `sudo rm -rf ${useThisVolume.mount}tmp/${
        zelAppSpecifications.name}TEMP`;
  }
  ...
  const execVerif =
      `sudo dd if=/dev/zero of=${zelappsFolder + zelAppSpecifications.name}/${
          zelAppSpecifications.name}VERTEMP bs=96636763 count=${
          zelAppSpecifications.hdd}0`; // 90%
  ...
  const execFinal = `sudo rm -rf ${zelappsFolder + zelAppSpecifications.name}/${
      zelAppSpecifications.name}VERTEMP`;
...
async function removeZelAppLocally(zelapp, res) {
  try {
    ...
    const execUnmount =
        `sudo umount ${zelappsFolder + zelAppSpecifications.name}`;
    ...
    const execDelete =
        `sudo rm -rf ${zelappsFolder + zelAppSpecifications.name}`;
...
async function checkZelAppMessageExistence(zelapphash) {
  try {
    ...
    const zelappResult =
        await serviceHelper
            .findOneInDatabase(zelappsDatabase, globalZelAppsMessages,
                               zelappsQuery, zelappsProjection)
            .catch((error) => {
              dbopen.close();
              throw error;
