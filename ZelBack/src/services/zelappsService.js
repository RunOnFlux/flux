...
async function createZelAppVolume(zelAppSpecifications, res) {
  ...
  try {
    ...
    let execDD = `sudo dd if=/dev/zero of=${useThisVolume.mount}/${
        zelappId}FLUXFSVOL bs=107374182 count=${
        zelAppSpecifications.hdd}0`; // eg /mnt/sthMounted/zelappTEMP
    if (useThisVolume.mount === '/') {
      execDD = `sudo dd if=/dev/zero of=${fluxDirPath}appvolumes/${
          zelappId}FLUXFSVOL bs=107374182 count=${
    ...
    let execFS =
        `sudo mke2fs -t ext4 ${useThisVolume.mount}/${zelappId}FLUXFSVOL`;
    if (useThisVolume.mount === '/') {
      execFS =
          `sudo mke2fs -t ext4 ${fluxDirPath}appvolumes/${zelappId}FLUXFSVOL`;
    }
    ...
    let execRemoveAlloc =
        `sudo rm -rf ${useThisVolume.mount}/${zelappId}FLUXFSVOL`;
    if (useThisVolume.mount === '/') {
      execRemoveAlloc =
          `sudo rm -rf ${fluxDirPath}appvolumes/${zelappId}FLUXFSVOL`;
    }
  ...
  try {
    ...
    if (!zelAppSpecifications) {
      ...
      zelAppSpecifications = await serviceHelper.findOneInDatabase(
          database, globalZelAppsInformation, zelappsQuery, zelappsProjection);
      if (!zelAppSpecifications) {
...
async function softRemoveZelAppLocally(zelapp, res) {
  ...
  const zelappRemovalResponse =
      serviceHelper.createDataMessage(`ZelApp ${zelapp} was partially removed`);
  log.info(zelappRemovalResponse);
  if (res) {
    res.write(serviceHelper.ensureString(zelappRemovalResponse));
  }
  ...
  try {
    ...
    dockerPullStream(zelAppSpecifications.repotag, res, async (error, dataLog) => {
      if (error) {
        ...
        const volumeOK =
            await createZelAppVolume(zelAppSpecifications, res).catch((errr) => {
              const errorResponse = serviceHelper.createErrorMessage(
                  errr.message || errr,
                  errr.name,
                  errr.code,
...
async function availableZelApps(req, res) {
  ...
  const zelapps = [
    ...
    },
    {
      ...
      repotag : 'zelcash/kadena-chainweb-node:2.3',
      ...
      hash : 'localSpecificationsVersion3', // hash of app message
...
async function checkApplicationNameConflicts(zelAppSpecFormatted) {
  ...
  const zelappExists =
      localApps.find((localApp) => localApp.name.toLowerCase() ===
                                   zelAppSpecFormatted.name.toLowerCase());
  if (zelappExists) {
    throw new Error(`ZelApp ${
        zelAppSpecFormatted
            .name} already assigned to local application. ZelApp has to be registered under different name.`);
  }
  if (zelAppSpecFormatted.name.toLowerCase() === 'share') {
    throw new Error(`ZelApp ${
        zelAppSpecFormatted
            .name} already assigned to Flux main application. ZelApp has to be registered under different name.`);
  }
  return true;
}
...
async function registerZelAppGlobalyApi(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      if (zelfluxCommunication.outgoingPeers.length <
              config.zelapps.minOutgoing ||
          zelfluxCommunication.incomingPeers.length <
              config.zelapps.minIncoming) {
        throw new Error(
  ...
  try {
    ...
    if (authorized) {
      ...
      const zelAppSpecifications =
          allZelApps.find((zelapp) => zelapp.name === applicationName);
      if (!zelAppSpecifications) {
...
async function getApplicationSpecifications(appName) {
  ...
  if (!zelappInfo) {
    const allZelApps = await availableZelApps();
    zelappInfo = allZelApps.find((zelapp) => zelapp.name.toLowerCase() ===
                                             appName.toLowerCase());
  }
  return zelappInfo;
}
        ...
        if (appDetails) {
          ...
          if (!removalInProgress) {
            ...
            log.warn(`Not starting ${
                stoppedApp} as of application removal in progress`);
          }
        }
      } catch (err) {
        log.error(err);
...
async function softRedeploy(zelappSpecs, res) {
  try {
    ...
    await serviceHelper.delay(config.zelapps.redeploy.delay *
  ...
  try {
    ...
    for (const installedApp of installedApps) {
      ...
      const randomNumber = Math.floor(
          (Math.random() * config.zelapps.redeploy.probability)); // 50%
...
async function zelShareDatabaseFileDelete(file) {
  try {
    ...
    await serviceHelper.findOneAndDeleteInDatabase(
        databaseZelShare, sharedCollection, queryZelShare, projectionZelShare);
    return true;
  } catch (error) {
    log.error(error);
    throw error;
...
async function zelShareDatabaseFileDeleteMultiple(pathstart) {
  try {
    ...
    await serviceHelper.removeDocumentsFromCollection(
        databaseZelShare, sharedCollection, queryZelShare);
    return true;
  } catch (error) {
    log.error(error);
    throw error;
...
async function zelShareDatabaseShareFile(file) {
  try {
    ...
    await serviceHelper.insertOneToDatabase(databaseZelShare, sharedCollection,
                                            fileDetail);
    return fileDetail;
  } catch (error) {
    log.error(error);
...
async function zelShareGetFolder(req, res) {
  try {
    ...
    if (authorized) {
      ...
      let sharedFiles =
          await zelShareSharedFiles().catch((error) => { log.error(error); });
