...
async function fluxDiscovery() {
  try {
    ...
    const maxCon = Math.max(maxPeers, maxNumberOfConnections); // have a maximum of 20 or 2% of
    ...
    while (outgoingConnections.length < minCon && index < 100) {
      ...
      if (ip) {
        ...
        const sameConnectedIp = currentIpsConnTried.find(
...
async function checkMyFluxAvailability(nodelist) {
  ...
  if (nodelist.length > 10) {
    ...
    if (
      typeof askingIP !== 'string' ||
      typeof myFluxIP !== 'string' ||
      myFluxIP === askingIP
    ) {
      return;
    }
      ...
      if (benchIpResponse.status === 'success') {
        ...
        if (benchMyIP && benchMyIP !== myIP) {
          ...
          setTimeout(() => {
            ...
            // the bench.
            daemonService.createConfirmationTransaction(); // on my pi for reference,
...
function isCommunicationEstablished(req, res) {
  let message;
  if (
    outgoingPeers.length + incomingPeers.length <
