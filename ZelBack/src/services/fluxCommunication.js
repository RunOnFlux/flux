...
const outgoingPeers =
    []; // array of objects containing ip, latency, lastPingTime
      ...
      try {
        ...
        if (client.readyState === WebSocket.OPEN) {
          if (!data) {
            ...
            const foundPeer = outgoingPeers.find(
                (peer) => peer.ip === client._socket.remoteAddress);
            if (foundPeer) {
          ...
          throw new Error(
              `Connection to ${client._socket.remoteAddress} is not open`);
        }
      } catch (e) {
  ...
  try {
    ...
    if (permanentMessage) {
      ...
      if (existingTemporaryMessage) {
        ...
        //   message.zelAppSpecifications, type: message.type, version:
        //   message.version, hash: message.hash, timestamp: message.timestamp,
  ...
  ws.on('message', async (msg) => {
    ...
    if (messageOK === true) {
      ...
      try {
        ...
        ws.close(1000,
                 'invalid message, blocked'); // close as of policy violation?
      } catch (e) {
        console.error(e);
      }
    }
  ...
  try {
    ...
    if (myNodePubKey) {
      ...
      if (nodeList.length === 0) {
        myNodePubKey = null;
        throw new Error(
            'Node no longer confirmed. Flux discovery is awaiting.');
      }
    } else {
    ...
    const minCon =
        Math.max(minPeers,
                 requiredNumberOfConnections); // awlays maintain at least 10 or
                                               // 1% of nodes whatever is higher
    const maxCon = Math.max(
        maxPeers, maxNumberOfConnections); // have a maximum of 20 or 2% of
                                           // nodes whatever is higher
    log.info(
    ...
    while (outgoingConnections.length < minCon && index < 100) {
      ...
      if (ip) {
        ...
        const sameConnectedIp =
            currentIpsConnTried.find((connectedIP) => connectedIP === ip);
    ...
    setTimeout(() => { fluxDiscovery(); }, 120 * 1000);
  }
}
  ...
  if (nodelist.length > 10) {
    ...
    if (resMyAvailability.data.status === 'error' ||
      ...
      if (benchIpResponse.status === 'success') {
        const benchMyIP =
            benchIpResponse.data.length > 5 ? benchIpResponse.data : null;
        if (benchMyIP && benchMyIP !== myIP) {
          myIP = benchMyIP;
          const restartNodeResponse =
              await daemonService.restartNodeBenchmarks();
          ...
          setTimeout(() => {
            // it will try to create a confirmation transaction after 6 minutes
            // of ask bench to restart, should be enough to restart and finish
            // the bench.
            daemonService
                .createConfirmationTransaction(); // on my pi for reference,
                                                  // running bench on micro sd
                                                  // card took 3 minutes to
                                                  // finish
          }, 4 * 60 * 1000);
          return;
        }
...
async function checkDeterministicNodesCollisions() {
  try {
    ...
    // another precatuion might be comparing node list on multiple nodes.
    ...
    setTimeout(() => { checkDeterministicNodesCollisions(); }, 60 * 1000);
  } catch (error) {
    log.error(error);
    setTimeout(() => { checkDeterministicNodesCollisions(); }, 120 * 1000);
  }
}
...
function isCommunicationEstablished(req, res) {
  let message;
  if (outgoingPeers.length + incomingPeers.length <
      config.fluxapps.minOutgoing + config.fluxapps.minIncoming) {
    message = serviceHelper.createErrorMessage(
        'Not enough connections established to Flux network');
  } else {
    message = serviceHelper.createSuccessMessage(
...
async function broadcastTemporaryAppMessage(message) {
   ...
   *     JSON.stringify(appSpecifications) + timestamp + signature))
