...
const coinbaseFusionIndexCollection =
    config.database.daemon.collections.coinbaseFusionIndex; // fusion
...
const addressTransactionIndexCollection =
    config.database.daemon.collections.addressTransactionIndex;
const scannedHeightCollection =
    config.database.daemon.collections.scannedHeight;
const fluxTransactionCollection =
    config.database.daemon.collections.fluxTransactions;
...
async function getSenderForFluxTx(txid, vout) {
  ...
  if (!txContent) {
    log.info(`Transaction ${txid} ${
        vout} not found in database. Falling back to previous Flux transaction`);
    const queryFluxTx = {
      $and : [
    ...
    txContent = await serviceHelper.findOneInDatabase(
        database, fluxTransactionCollection, queryFluxTx, projectionFluxTx);
  }
  if (!txContent) {
    log.warn(
    ...
    await Promise.all(transactionDetail.vout.map(async (vout, index) => {
      ...
      if (vout.scriptPubKey.addresses) {
        ...
        await serviceHelper.insertOneToDatabase(database, utxoIndexCollection,
                                                utxoDetail);
        // track coinbase txs for additional rewards on paralel chains for
        // fusion
        if (coinbase &&
            height > 825000) { // 825000 is snapshot, 825001 is first block
                               // eligible for rewards on other chains
          await serviceHelper.insertOneToDatabase(
              database, coinbaseFusionIndexCollection, utxoDetail);
        }
      }
    }));
...
async function processBlock(blockHeight) {
  try {
    ...
    if (!syncStatus.data.synced) {
      setTimeout(() => { processBlock(blockHeight); }, 2 * 60 * 1000);
      return;
    }
      ...
      if (tx.version < 5 && tx.version > 0) {
        ...
        tx.vout.forEach((receiver) => {
          if (receiver.scriptPubKey.addresses) { // count for messages
            ...
            if (receiver.scriptPubKey.addresses[0] ===
        ...
        if (isFluxAppMessageValue >= 1e8 && message.length === 64 &&
            blockDataVerbose.height >=
                config.fluxapps.epochstart) { // min of 1 flux had to be paid
                                              // for us bothering checking
          const appTxRecord = {
            txid : tx.txid,
            height : blockDataVerbose.height,
            hash : message,
            value : isFluxAppMessageValue,
            message : false, // message is boolean saying if we already have it
                             // stored as permanent message
          };
          await serviceHelper.insertOneToDatabase(
              database, appsHashesCollection, appTxRecord);
          appsService.checkAndRequestApp(
              message, tx.txid, blockDataVerbose.height, isFluxAppMessageValue);
        }
      }
      // tx version 5 are flux transactions. Put them into flux
      if (tx.version === 5) {
        // todo include to daemon better information about hash and index and
        ...
        const senderInfo =
            await getSenderForFluxTx(partialCollateralHash, collateralIndex);
        const fluxTxData = {
          txid : tx.txid,
          version : tx.version,
          type : tx.type,
        ...
        await serviceHelper.insertOneToDatabase(
            database, fluxTransactionCollection, fluxTxData);
      }
    }));
    ...
    if (blockHeight % config.fluxapps.expireFluxAppsPeriod === 0) {
      ...
      log.info(`ADDR documents: ${resultB.size}, ${resultB.count}, ${
          resultB.avgObjSize}`);
      log.info(`FLUX documents: ${resultC.size}, ${resultC.count}, ${
          resultC.avgObjSize}`);
      const resultFusion = await serviceHelper.collectionStats(
          database, coinbaseFusionIndexCollection);
      log.info(`Fusion documents: ${resultFusion.size}, ${
          resultFusion.count}, ${resultFusion.avgObjSize}`);
...
async function restoreDatabaseToBlockheightState(height,
  ...
  await serviceHelper.removeDocumentsFromCollection(
      database, coinbaseFusionIndexCollection, query);
  ...
  await serviceHelper.removeDocumentsFromCollection(
      database, fluxTransactionCollection, query);
  // restore appsHashes collection
  await serviceHelper.removeDocumentsFromCollection(
      database, appsHashesCollection, query);
  if (rescanGlobalApps === true) {
    const databaseGlobal = dbopen.db(config.database.appsglobal.database);
    log.info('Rescanning Apps!');
    await serviceHelper.removeDocumentsFromCollection(
        databaseGlobal, config.database.appsglobal.collections.appsMessages,
        query);
    await serviceHelper.removeDocumentsFromCollection(
        databaseGlobal, config.database.appsglobal.collections.appsInformation,
        query);
  }
...
// do a deepRestore of 100 blocks if daemon if enouncters an error (mostly flux
// daemon was down) or if its initial start of flux use reindexGlobalApps with
// caution!!!
async function initiateBlockProcessor(restoreDatabase, deepRestore,
  ...
  try {
    ...
    if (!syncStatus.data.synced) {
      setTimeout(() => {
        initiateBlockProcessor(restoreDatabase, deepRestore,
                               reindexOrRescanGlobalApps);
      }, 2 * 60 * 1000);
      return;
    }
    ...
    if (scannedBlockHeight === 0) {
      ...
      const resultC = await serviceHelper
                          .dropCollection(database, fluxTransactionCollection)
                          .catch((error) => {
                            if (error.message !== 'ns not found') {
                              throw error;
                            }
      ...
      const resultD =
          await serviceHelper.dropCollection(database, appsHashesCollection)
              .catch((error) => {
                if (error.message !== 'ns not found') {
                  throw error;
                }
      ...
      const resultFusion =
          await serviceHelper
              .dropCollection(database, coinbaseFusionIndexCollection)
              .catch((error) => {
                if (error.message !== 'ns not found') {
                  throw error;
                }
      ...
      await database.collection(coinbaseFusionIndexCollection)
          .createIndex(
              {txid : 1, vout : 1},
              {name : 'query for getting coinbase fusion utxo', unique : true});
      await database.collection(coinbaseFusionIndexCollection)
          .createIndex({txid : 1, vout : 1, satoshis : 1}, {
            name : 'query for getting coinbase fusion utxo for zelnode tx',
            unique : true
          });
      await database.collection(coinbaseFusionIndexCollection)
          .createIndex({address : 1},
                       {name : 'query for addresses coinbase fusion utxo'});
      await database.collection(coinbaseFusionIndexCollection)
          .createIndex({scriptPubKey : 1},
                       {name : 'query for scriptPubKey coinbase fusion utxo'});
      ...
      await database.collection(fluxTransactionCollection)
          .createIndex({ip : 1}, {
            name :
      ...
      await database.collection(appsHashesCollection).createIndex({txid : 1}, {
        name : 'query for getting txid'
      });
      await database.collection(appsHashesCollection)
          .createIndex({height : 1}, {name : 'query for getting height'});
      await database.collection(appsHashesCollection).createIndex({hash : 1}, {
        name : 'query for getting app hash'
      });
      await database.collection(appsHashesCollection)
      ...
      await databaseGlobal
          .collection(config.database.appsglobal.collections.appsMessages)
          .createIndex({hash : 1}, {
      ...
      await databaseGlobal
          .collection(config.database.appsglobal.collections.appsMessages)
          .createIndex({'appSpecifications.name' : 1}, {
            name : 'query for getting app message based on zelapp specs name'
          }); // , unique: true
      await databaseGlobal
          .collection(config.database.appsglobal.collections.appsMessages)
          .createIndex({'appSpecifications.owner' : 1}, {
            name : 'query for getting app message based on zelapp specs owner'
          });
      await databaseGlobal
          .collection(config.database.appsglobal.collections.appsMessages)
          .createIndex({'appSpecifications.repotag' : 1},
                       {name : 'query for getting app message based on image'});
      await databaseGlobal
    ...
    if (daemonHeight > scannedBlockHeight) {
      if (scannedBlockHeight !== 0 && restoreDatabase === true) {
        ...
        if (daemonGetChainTips.status !== 'success') {
          throw new Error(daemonGetChainTips.data.message ||
                          daemonGetInfo.data);
        }
...
async function getAllFusionCoinbase(req, res) {
  try {
    ...
    const results = await serviceHelper.findInDatabase(
        database, coinbaseFusionIndexCollection, query, projection);
    const resMessage = serviceHelper.createDataMessage(results);
    res.json(resMessage);
  } catch (error) {
    log.error(error);
...
async function getAllFluxTransactions(req, res) {
  try {
    ...
    const results = await serviceHelper.findInDatabase(
        database, fluxTransactionCollection, query, projection);
    const resMessage = serviceHelper.createDataMessage(results);
    res.json(resMessage);
  } catch (error) {
    log.error(error);
