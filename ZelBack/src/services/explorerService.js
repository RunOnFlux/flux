...
async function getSenderForFluxTx(txid, vout) {
  ...
  if (!txContent) {
    log.info(
      `Transaction ${txid} ${vout} not found in database. Falling back to previous Flux transaction`
    );
    const queryFluxTx = {
      $and: [
    ...
    await Promise.all(
      transactionDetail.vout.map(async (vout, index) => {
        ...
        if (vout.scriptPubKey.addresses) {
          ...
          // fusion
          if (coinbase && height > 825000) {
            // 825000 is snapshot, 825001 is first block
            // eligible for rewards on other chains
            await serviceHelper.insertOneToDatabase(
              database,
              coinbaseFusionIndexCollection,
              utxoDetail
            );
          }
        }
...
async function processBlock(blockHeight) {
  try {
    ...
    await Promise.all(
      transactions.map(async (tx) => {
        ...
        if (tx.version < 5 && tx.version > 0) {
          ...
          tx.vout.forEach((receiver) => {
            if (receiver.scriptPubKey.addresses) {
              ...
              if (
                receiver.scriptPubKey.addresses[0] === config.fluxapps.address
              ) {
          ...
          if (
            isFluxAppMessageValue >= 1e8 &&
            message.length === 64 &&
            blockDataVerbose.height >= config.fluxapps.epochstart
          ) {
            // min of 1 flux had to be paid
        ...
        if (tx.version === 5) {
          ...
          const senderInfo = await getSenderForFluxTx(
    ...
    if (blockHeight % config.fluxapps.expireFluxAppsPeriod === 0) {
      ...
      log.info(
        `UTXO documents: ${result.size}, ${result.count}, ${result.avgObjSize}`
      );
      log.info(
        `ADDR documents: ${resultB.size}, ${resultB.count}, ${resultB.avgObjSize}`
      );
      log.info(
        `FLUX documents: ${resultC.size}, ${resultC.count}, ${resultC.avgObjSize}`
      );
      ...
      log.info(
        `Fusion documents: ${resultFusion.size}, ${resultFusion.count}, ${resultFusion.avgObjSize}`
      );
...
async function restoreDatabaseToBlockheightState(
  ...
  await serviceHelper.updateInDatabase(
    database,
    addressTransactionIndexCollection,
    queryForAddresses,
    projection
  ...
  try {
    ...
    if (scannedBlockHeight === 0) {
      ...
      const resultD = await serviceHelper
        .dropCollection(database, appsHashesCollection)
        .catch((error) => {
          if (error.message !== 'ns not found') {
            throw error;
          }
        });
      const resultFusion = await serviceHelper
      ...
      await database
        .collection(coinbaseFusionIndexCollection)
        .createIndex(
          { txid: 1, vout: 1 },
          { name: 'query for getting coinbase fusion utxo', unique: true }
        );
      await database.collection(coinbaseFusionIndexCollection).createIndex(
        { txid: 1, vout: 1, satoshis: 1 },
        {
      ...
      await database.collection(fluxTransactionCollection).createIndex(
        { ip: 1 },
        {
          name:
      ...
      await database.collection(appsHashesCollection).createIndex(
        { txid: 1 },
        {
      ...
      await database
        .collection(appsHashesCollection)
      ...
      if (reindexOrRescanGlobalApps === true) {
        ...
        const resultG = await serviceHelper
          .dropCollection(
            databaseGlobal,
    ...
    if (daemonHeight > scannedBlockHeight) {
      if (scannedBlockHeight !== 0 && restoreDatabase === true) {
        ...
        if (daemonGetChainTips.status !== 'success') {
          throw new Error(
            daemonGetChainTips.data.message || daemonGetInfo.data
          );
        }
