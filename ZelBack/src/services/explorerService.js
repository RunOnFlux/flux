...
async function getSenderForZelNodeTx(txid, vout) {
  ...
  if (!txContent) {
    ...
    txContent = await serviceHelper
                    .findOneInDatabase(database, zelnodeTransactionCollection,
                                       queryZelNode, projectionZelNode)
                    .catch((error) => {
                      log.error(error);
                      throw error;
                    });
  ...
  try {
    ...
    if (scannedBlockHeight === 0) {
      ...
      await databaseGlobal
          ...
          }); // we need to know the hash of the last message update which is
              // the true identifier
    ...
    log.error(error);
    setTimeout(() => { initiateBlockProcessor(true, true); }, 15 * 60 * 1000);
  }
}
...
async function getAddressTransactions(req, res) {
  ...
  const results =
      await serviceHelper
          .distinctDatabase(database, addressTransactionIndexCollection,
                            distinct, query)
          .catch((error) => {
            const errMessage = serviceHelper.createErrorMessage(
                error.message, error.name, error.code);
