...
async function createAppVolume(appSpecifications, res) {
  ...
  try {
    ...
    let execDD = `sudo dd if=/dev/zero of=${useThisVolume.mount}/${
        appId}FLUXFSVOL bs=107374182 count=${
        appSpecifications
            .hdd}0 status=progress`; // eg /mnt/sthMounted/zelappTEMP
    if (useThisVolume.mount === '/') {
      execDD = `sudo dd if=/dev/zero of=${fluxDirPath}appvolumes/${
          appId}FLUXFSVOL bs=107374182 count=${
          appSpecifications.hdd}0 status=progress`; // if root mount then temp
                                                    // file is /tmp/zelappTEMP
    }
    let iterationAlloc = 0;
    global.allocationInterval = setInterval(() => {
      iterationAlloc += 1;
      const allocateSpaceB = {
        status : `Space allocation is running for ${
            20 * iterationAlloc}s. This may take quite some time...`,
      };
    ...
    const execVerif = `sudo dd if=/dev/zero of=${appsFolder + appId}/${
        appId}VERTEMP bs=96636763 count=${
        appSpecifications.hdd}0 status=progress`; // 90%
    let interationVerif = 0;
    global.verificationInterval = setInterval(() => {
      interationVerif += 1;
      const spaceVerifB = {
        status : `Space verification is running for ${
            20 * interationVerif}s. This may take quite some time...`,
      };
    ...
    const finaliseSpace2 = {
      status :
          `Space for Flux App ${appSpecifications.name} created and assigned.`,
    };
...
async function softRemoveAppLocally(app, res) {
  // remove app from local machine.
  // find in database, stop app, remove container, close port, remove from
    ...
    dockerPullStream(appSpecifications.repotag, res, async (error, dataLog) => {
      if (error) {
        ...
        if (appSpecifications.ports) {
          ...
          // v1 compatibility
          const portResponse =
              await fluxCommunication.allowPort(appSpecifications.port);
          if (portResponse.status === true) {
            const portStatus = {
              status : 'Port OK',
            };
...
async function storeAppTemporaryMessage(message, furtherVerification = false) {
  ...
  if (furtherVerification) {
    if (message.type === 'zelappregister' ||
      ...
      await verifyAppHash(message);
      await ensureCorrectApplicationPort(specifications);
      // verify that app exists, does not change repotag and is signed by app
      // owner.
      const db = serviceHelper.databaseConnection();
      const database = db.db(config.database.appsglobal.database);
      // may throw
  ...
  req.on('end', async () => {
    try {
      ...
      if (fluxCommunication.outgoingPeers.length <
              config.fluxapps.minOutgoing ||
          fluxCommunication.incomingPeers.length <
              config.fluxapps.minIncoming) {
        throw new Error(
      ...
      // validity, if they are within limits, have propper ports, repotag
      // exists, string lengths, specs are ok
      ...
      // in appsTemporaryMessages. with 1 hours expiration time. Broadcast this
...
async function updateAppGlobalyApi(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      // Note. Actually signature, timestamp is not needed. But we require it
      // only to verify that user indeed has access to the private key of the
      // owner zelid. name and ports HAVE to be unique for application. Check if
      // they dont exist in global database first lets check if all fields are
      // present and have propper format excpet tiered and teired specifications
      // and those can be ommited
      let {appSpecification} = processedBody;
...
async function updateAppSpecifications(appSpecs) {
  try {
    ...
    //   '["--allow","0/0","--web-allow","0/0"]', // [] containerData:
    //   '/config', cpu: 0.5, ram: 500, hdd: 5, tiered: true, cpubasic: 0.5,
    //   rambasic: 500,
    //   hddbasic: 5,
    //   cpusuper: 1,
    //   ramsuper: 1000,
...
async function updateAppSpecsForRescanReindex(appSpecs) {
  ...
  //   enviromentParameters: '["USER=foldingUser", "TEAM=262156",
  //   "ENABLE_GPU=false", "ENABLE_SMP=true"]', // [] commands:
  //   '["--allow","0/0","--web-allow","0/0"]', // [] containerData: '/config',
  //   cpu: 0.5,
  //   ram: 500,
  //   hdd: 5,
  //   tiered: true,
...
// hash of app information, txid it was in, height of blockchain containing the
...
// function that drops global apps information and goes over all global apps
// messages and reconsturcts the global apps information. Further creates
// database indexes
...
async function checkSynced() {
  try {
    // check if flux database is synced with daemon database (equal or -1
    // inheight)
    const daemonGetInfo = await daemonService.getInfo();
    let daemonHeight;
    if (daemonGetInfo.status === 'success') {
...
async function trySpawningGlobalApplication() {
  try {
    ...
    if (tier === 'basic') {
      log.info(
          'Basic node detected. Global applications will not be installed');
    }
...
// oldpath is relative path to default fluxshare directory; newname is just a
// new name of folder/file
