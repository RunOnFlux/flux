...
async function createAppVolume(appSpecifications, res) {
  ...
  try {
    ...
    if (appSpecifications.userVolumeOwnership) {
      const execUserOwner = `sudo chown ${
        appSpecifications.userVolumeOwnership
      }.${appSpecifications.userVolumeOwnership} ${appsFolder + appId} -R`;
...
async function availableApps(req, res) {
  ...
  const apps = [
    ...
    },
    {
      ...
      hddbamf: 5,
      enviromentParameters: [
        `FOLD_USER=${userconfig.initial.zelid}`,
        'FOLD_TEAM=262156',
        'FOLD_ANON=false',
      ],
      commands: [],
  ...
  try {
    ...
    if (
      runningApps.data.find(
        (app) => app.Names[0].substr(5, app.Names[0].length) === randomApp
      )
    ) {
...
async function checkAndNotifyPeersOfRunningApps() {
  try {
    ...
    // kadena and folding is old naming scheme having /zel.  all global
    // application start with /flux
