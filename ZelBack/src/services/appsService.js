  ...
  for (let i = 0; i < appsMessages.length; i += 1) {
    ...
    if (result && result.broadcastedAt && result.broadcastedAt >= newAppRunningMessage.broadcastedAt) {
      ...
      if (message.ip === '94.16.104.218') {
        log.info(`App Running Message from IP 94.16.104.218 - found a message that was already stored result.broadcastedAt: ${result.broadcastedAt} newAppRunningMessage.broadcastedAt: ${newAppRunningMessage.broadcastedAt}`);
      }
      messageNotOk = true;
      break;
    }
    ...
    if (message.ip === '94.16.104.218') {
      log.info('Message from IP 94.16.104.218 is going to be stored');
    }
    // eslint-disable-next-line no-await-in-loop
