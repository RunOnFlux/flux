...
async function checkAppSecrets(appName, appComponentSpecs, registration = false) {
  ...
  const projection = { projection: { _id: 0 } };
  const results = await dbHelper.findInDatabase(database, globalAppsInformation, query, projection);
  log.info(`checkAppSecrets: ${appName} lets search on db for the secrets: ${JSON.stringify(appComponentSpecs.secrets)}`);
  // eslint-disable-next-line no-restricted-syntax
      ...
      for (const component of app.compose) {
        if (component.secrets.length > 0) {
          if (app.name === 'BrokerNode1677472532536') {
            log.info(`checkAppSecrets: component ${component.name} of appName same secrets on app: ${app.name} secrets: ${JSON.stringify(component.secrets)}`);
          }
          if (JSON.stringify(component.secrets) === JSON.stringify(appComponentSpecs.secrets)) {
