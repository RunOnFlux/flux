...
async function createAppVolume(appSpecifications, res) {
  ...
  try {
    ...
    if (useThisVolume.mount === '/') {
      execDD = `sudo dd if=/dev/zero of=${fluxDirPath}appvolumes/${appId}FLUXFSVOL bs=107374182 count=${appSpecifications.hdd}0 status=progress`; // if root mount then temp
    ...
    global.allocationInterval = setInterval(() => {
      ...
      const allocateSpaceB = {
        status: `Space allocation is running for ${
          20 * iterationAlloc
        }s. This may take quite some time...`,
      };
...
async function registerAppGlobalyApi(req, res) {
  ...
  req.on('end', async () => {
    try {
      ...
      if (
        fluxCommunication.outgoingPeers.length < config.fluxapps.minOutgoing ||
        fluxCommunication.incomingPeers.length < config.fluxapps.minIncoming
      ) {
        throw new Error(
