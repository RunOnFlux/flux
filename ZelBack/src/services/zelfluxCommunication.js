    ...
    if (!zelnodeList) {
      ...
      if (zelcashZelNodeList.status === 'success') {
        ...
        zelnodeList.forEach((item) => {
          myCache.set(`zelnodeList${serviceHelper.ensureString(item.pubkey)}`,
                      [ item ]);
        });
...
function handleIncomingConnection(ws, req, expressWS) {
  ...
  ws.on('message', async (msg) => {
    ...
    if (messageOK === true) {
          ...
          if (msgObj.data.type === 'zelappregister' ||
            ...
            try {
              ...
              console.log(error);
                     ...
                     msgObj.data.message ===
                         'pong') { // we know that data exists. This is
                                   // measuring rtt from incoming conn
            ...
            try {
              ws.send(`Flux ${
                  userconfig.initial.ipaddress} says message received!`);
            } catch (error) {
              console.log(error);
            }
          }
      ...
      // connection and add him to blocklist
      try {
