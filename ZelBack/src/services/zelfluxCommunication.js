...
async function respondWithAppMessage(message, ws) {
  ...
  if (permanentMessage) {
    // message exists in permanent storage. Create a message and broadcast it to
    // the fromIP peer const permanentZelAppMessage = {
    //   type: messageType,
    //   version: typeVersion,
...
async function initiateAndHandleConnection(ip) {
  ...
  websocket.onmessage =
      async (evt) => { // TODO message handling outgoing connections function
...
function isCommunicationEstablished(req, res) {
  ...
  if (outgoingPeers.length < 5) {
    ...
    message =
        serviceHelper.createErrorMessage('Not enough incomming connections');
  } else {
    message = serviceHelper.createSuccessMessage(
