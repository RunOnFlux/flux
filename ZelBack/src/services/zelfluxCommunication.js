...
async function isFluxAvailable(ip) {
  ...
  try {
    const fluxResponse = await serviceHelper.axiosGet(
        `http://${ip}:${config.server.apiport}/zelflux/version`, axiosConfig);
...
async function deterministicZelNodeList(filter) {
  try {
    ...
    let zelnodeList = [];
    zelnodeList =
        myCache.get(`zelnodeList${serviceHelper.ensureString(filter)}`);
    if (!zelnodeList) {
      const zelcashZelNodeList =
          await zelcashService.viewDeterministicZelNodeList(request);
      if (zelcashZelNodeList.status === 'success') {
        zelnodeList = zelcashZelNodeList.data || [];
        myCache.set(`zelnodeList${serviceHelper.ensureString(filter)}`,
                    zelnodeList); // can be zelnodeListundefined for main list
      }
    }
    return zelnodeList || [];
  } catch (error) {
...
// return boolean
async function verifyFluxBroadcast(data, obtainedZelNodeList,
                                   currentTimeStamp) {
  ...
  if (!zelnode) {
    const zl =
        await deterministicZelNodeList(pubKey); // this itself is sufficient.
    if (zl.length === 1) {
  ...
  if (!zelnode) { // if filtering fails, fetch all the list and run find method
    // eslint-disable-next-line no-param-reassign
    obtainedZelNodeList =
        await deterministicZelNodeList(); // support for daemons that do not
                                          // have filtering via public key
    zelnode = await obtainedZelNodeList.find((key) => key.pubkey === pubKey);
  }
  if (!zelnode) {
    return false;
...
async function verifyOriginalFluxBroadcast(data, obtainedZelNodeList,
                                           currentTimeStamp) {
  // eslint-disable-next-line no-param-reassign
  const dataObj = serviceHelper.ensureObject(data);
  const {timestamp} = dataObj; // ms
  ...
  const verified =
      await verifyFluxBroadcast(data, obtainedZelNodeList, currentTimeStamp);
  return verified;
}
...
async function serialiseAndSignZelFluxBroadcast(dataToBroadcast, privatekey) {
  ...
  // message contains version, timestamp, pubKey, signature and data. Data is a
  // stringified json. Signature is signature of
  // version+stringifieddata+timestamp signed by the priv key corresponding to
  // pubkey attached data object contains version, timestamp of signing,
  // signature, pubKey, data object. further data object must at least contain
  // its type as string to determine it further.
  const dataObj = {
    version,
    timestamp,
    pubKey,
...
async function handleZelAppMessages(message, fromIP) {
  try {
    ...
    const rebroadcastToPeers =
        await zelappsService.storeZelAppTemporaryMessage(message.data, true);
...
async function handleZelAppRunningMessage(message, fromIP) {
  try {
    ...
    const zelappsService = require('./zelappsService');
    const rebroadcastToPeers =
        await zelappsService.storeZelAppRunningMessage(message.data, true);
    if (rebroadcastToPeers === true) {
      const messageString = serviceHelper.ensureString(message);
      const wsListOut = outgoingConnections.filter(
          (client) => client._socket.remoteAddress !== fromIP);
...
async function respondWithAppMessage(message, ws) {
  try {
    ...
    const permanentMessage =
        await zelappsService.checkZelAppMessageExistence(message.data.hash);
    if (permanentMessage) {
      // message exists in permanent storage. Create a message and broadcast it
      // to the fromIP peer const permanentZelAppMessage = {
      //   type: messageType,
      //   version: typeVersion,
      ...
      const existingTemporaryMessage =
          await zelappsService.checkZelAppTemporaryMessageExistence(
              message.data.hash);
      ...
      // requesting it from other peers soon too.
    }
  } catch (error) {
    log.error(error);
  }
...
function handleIncomingConnection(ws, req, expressWS) {
  // now we are in connections state. push the websocket to our
  // incomingconnections
  ...
  ws.on('message', async (msg) => {
    ...
    if (messageOK === true && timestampOK === true) {
      try {
        ...
        if (msgObj.data.type === 'zelappregister' ||
        ...
        } else if (msgObj.data.type === 'HeartBeat' &&
                   msgObj.data.message === 'ping') { // we know that data exists
          const newMessage = msgObj.data;
          newMessage.message = 'pong';
          const pongResponse =
              await serialiseAndSignZelFluxBroadcast(newMessage);
          try {
            ws.send(pongResponse);
          } catch (error) {
            console.log(error);
                   ...
                   msgObj.data.message ===
                       'pong') { // we know that data exists. This is measuring
                                 // rtt from incoming conn
          const newerTimeStamp =
              Date.now(); // ms, get a bit newer time that has passed
          ...
          try {
            ws.send(
                `Flux ${userconfig.initial.ipaddress} says message received!`);
          } catch (error) {
            console.log(error);
          }
        }
      ...
      // we dont like this peer as it sent wrong message. Lets close the
      // connection
      try {
...
async function broadcastMessageToOutgoing(dataToBroadcast) {
  const serialisedData =
      await serialiseAndSignZelFluxBroadcast(dataToBroadcast);
...
async function getRandomConnection() {
  ...
  // const zelnodeList = ['157.230.249.150', '94.177.240.7', '89.40.115.8',
  // '94.177.241.10', '54.37.234.130', '194.182.83.182']; const zlLength =
  // zelnodeList.length; const randomNode = Math.floor((Math.random() *
  // zlLength)); // we do not really need a 'random' const ip =
  // zelnodeList[randomNode];
  ...
  websocket.onmessage = async (evt) => {
    ...
    if (messageOK === true) {
      ...
      if (msgObj.data.type === 'zelappregister' ||
        ...
        const newerTimeStamp = Date.now(); // ms, get a bit newer time that has
                                           // passed verification of broadcast
        const rtt = newerTimeStamp - msgObj.data.timestamp;
        const foundPeer = outgoingPeers.find((peer) => peer.ip === conIP);
        if (foundPeer) {
          const peerIndex = outgoingPeers.indexOf(foundPeer);
...
async function fluxDisovery() {
  ...
  // fast connect another peer as we do not have even enough connections to
  // satisfy min or wait 1 min.
  setTimeout(() => { fluxDisovery(); },
    ...
    const resMyAvailability =
        await serviceHelper
            .axiosGet(`http://${askingIP}:${
                config.server.apiport}/zelflux/checkfluxavailability/${myIP}`)
            .catch((error) => {
...
async function checkDeterministicNodesCollisions() {
  try {
    ...
    // another precatuion might be comparing zelnode list on multiple zelnodes.
    // evaulate in the future
    ...
    if (myIP !== null) {
      ...
      if (zelnodeStatus.status ===
          'success') { // different scenario is caught elsewhere
        const myCollateral = zelnodeStatus.data.collateral;
        const myZelNode =
            result.find((zelnode) => zelnode.collateral === myCollateral);
        if (result.length > 1) {
          log.warn('Multiple ZelNode instances detected');
          if (myZelNode) {
            const myBlockHeight =
                myZelNode.readded_confirmed_height ||
                myZelNode.confirmed_height; // todo we may want to introduce new
                                            // readded heights and readded
                                            // confirmations
            const filterEarlierSame = result.filter(
                (zelnode) => (zelnode.readded_confirmed_height ||
                              zelnode.confirmed_height) <= myBlockHeight);
  ...
  if (authorized === true) {
    ...
    if (portResponseOK.status === true) {
      ...
      response = serviceHelper.createErrorMessage(
          `Unkown error while opening port ${port}`);
    }
  } else {
    response = serviceHelper.errUnauthorizedMessage();
  }
...
async function broadcastTemporaryZelAppMessage(message) {
   ...
   * @param hash string - messageHash(type + version +
   *     JSON.stringify(zelAppSpecifications) + timestamp + signature))
