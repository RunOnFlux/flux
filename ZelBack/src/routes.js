...
module.exports = (app, expressWs) => {
  ...
  app.get('/explorer/rescan/:blockheight?',
          (req, res) => { explorerService.rescanExplorer(req, res); });

  app.get(
      '/zelapps/zelappregister/:repotag?/:name?/:owner?/:cpus?/:ram?/:space?/:port?/:ip?/:envvars?/:privacylevel?',
      (req, res) => { // privacy level means restrictions of calls. envvars can
                      // have privacy to be/not to be exposed // TODO make me
                      // post, needs redoing
  ...
  app.get('/zelapps/zelappremove/:zelapp?',
          (req, res) => { zelappsService.removeZelAppLocally(req, res); });
