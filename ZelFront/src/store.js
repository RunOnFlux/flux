...
const ALL_SECTIONS	= [ 'adminSection', 'appsSection', 'benchmarkSection', 'daemonSection', 'dashboardSection', 'explorerSection', 'nodeSection' ];

Vue.use(Vuex);


const nullSections = (state) => {
	for (const section of ALL_SECTIONS ) state[section] = null;
};
  ...
  state: {
	...
	mutations: {
		...
		setAdminSection(state, section) {
			setSection( 'adminSection', state, section );
		},
		setAppsSection(state, section) {
			setSection( 'appsSection', state, section );
		},
		setBenchmarkSection(state, section) {
			setSection( 'benchmarkSection', state, section );
		},
		setDaemonSection(state, section) {
			setSection( 'daemonSection', state, section );
		},
		setDashboardSection(state, section) {
			setSection( 'dashboardSection', state, section );
		},
		setExplorerSection(state, section) {
			setSection( 'explorerSection', state, section );
		},
		setNodeSection(state, section) {
			setSection( 'nodeSection', state, section );
		},
