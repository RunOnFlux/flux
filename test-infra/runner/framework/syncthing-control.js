...
// EMPTY global index (globalBytes 0) with local changes present - the data-loss
// trap: completion reads a vacuous 100% (needBytes 0), so the percentage-based
// isSynced is true while receiveOnlyChangedFiles reveals the local-only data. A
// node holding the only copy before peers reconnect (reboot/partition) sits here;
// the gate must NOT revert/promote against it (db/revert here deletes the only copy).
