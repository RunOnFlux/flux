...
if [ "$FLUX_SYNCTHING_MODE" = "binary" ]; then
  ...
  #
  # WHO starts it depends on the node type, and FLUXOS_PATH is the same signal
  # FluxOS reads to decide. Set, FluxOS takes the node for ArcaneOS and leaves
  # supervision to the OS - so the harness stands in for the OS here. Unset, it
  # is a legacy node and FluxOS supervises the daemon itself, so this must keep
  # its hands off or there would be two.
  #
  # Read the same variable FluxOS reads, not a different one that happens to be
  # set alongside it: keyed on two signals, a node with one of them gets either
  # two syncthings or none.
