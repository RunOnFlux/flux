...
if [[ -f "$COIN_PATH/zelcashd" ]]; then
    ...
    sudo rm "$COIN_PATH/zelcash"* >/dev/null 2>&1 && sleep 1
    sudo apt-get purge zelcash -y >/dev/null 2>&1 && sleep 1
...
if [[ -f "$COIN_PATH/$COIN_DAEMON" ]]; then
    ...
    sudo rm "$COIN_PATH/$COIN_DAEMON" >/dev/null 2>&1 && sleep 1
    sudo rm "$COIN_PATH/$COIN_CLI" >/dev/null 2>&1 && sleep 1
    sudo rm "$COIN_PATH/flux-tx" >/dev/null 2>&1 && sleep 1
    sudo rm "$COIN_PATH/flux-fetch-params.sh" >/dev/null 2>&1 && sleep 1
