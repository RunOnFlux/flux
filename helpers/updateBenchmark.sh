...
if [[ "$(lsb_release -cs)" == "xenial" ]]; then
  ...
  if gpg -k --keyring /usr/share/keyrings/flux-archive-keyring.gpg Zel > /dev/null 2>&1; then
    ...
    # echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/flux-archive-keyring.gpg] https://apt.fluxos.network/ $(lsb_release -cs) main" | sudo tee --append /etc/apt/sources.list.d/flux.list > /dev/null 2>&1
    sudo apt-get update -y
    sudo apt-get install "$BENCH_NAME" -y
    sudo chmod 755 "$COIN_PATH/"* && sleep 2
