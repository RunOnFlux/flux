    ...
    const initZelcore = async () => {
      try {
        ...
        if (window.zelcore) {
          ...
          window.open(`zel:?action=sign&message=${dataToSign.value}&icon=https%3A%2F%2Fraw.githubusercontent.com%2Frunonflux%2Fflux%2Fmaster%2FzelID.svg&callback=${callbackValue()}`);
        }
      } catch (error) {
        showToast('warning', 'Failed to sign message, please try again.');
      }
